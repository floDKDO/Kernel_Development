...
Exemple : ```gcc -static -o prog main.o -L ./ -ltest``` => lier la librairie statique (option ```-static``` empêche à gcc de lier des librairies partagées à notre programme) libtest.a (l'option ```-l``` doit être directement suivie du nom de la librarie sans le préfixe "lib" et sans l'extension .a) se trouvant dans le répertoire courant (option ```-L```) au fichier objet main.o pour former l'exécutable prog.
...
Voici quelques exemples de librairies statiques du langage C (présentes dans le dossier */usr/lib/x86_64-linux-gnu* sur ma machine) : 
